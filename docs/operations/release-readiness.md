...
- `packages/observability` now records local latency, token, cost, and tool-call metric
  records with redacted dimensions, and `pnpm eval:smoke` runs deterministic local
  scenarios for tool safety, docs generation, memory recall, and recovery without a
  hosted observability or external eval backend.
...
| Local latency, token, cost, and tool-call metrics plus deterministic regression eval smokes exist. | Supported for current local fixtures | `packages/observability/src/index.mjs`, `packages/observability/test/observability.test.mjs`, `evals/smoke.mjs`, `pnpm observability:test`, `pnpm eval:smoke`. | "The repo records local redacted usage metrics and runs deterministic smoke scenarios for tool safety, docs generation, memory recall, and recovery; hosted observability and external eval backends are not claimed." |
